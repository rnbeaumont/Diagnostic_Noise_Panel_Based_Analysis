	...
	foreach my $key (keys %panel_genes){
		...
		for(my $i=0;$i<@F;$i++){
			if(!exists $genes_found{$key."_".$F[$i]} && ($F[$i] eq "monoallelic" || $F[$i] eq "biallelic")){
				print $out join("\t",$key,"NA","NA",$F[$i],"conflicting","0","0",$g_length{$gnames{$key}},$g_length_tot{$gnames{$key}})."\n"; # need gene lengths here - this requires ensembl ID and transcript ID
				print $out join("\t",$key,"NA","NA",$F[$i],"benign","0","0",$g_length{$gnames{$key}},$g_length_tot{$gnames{$key}})."\n"; # need gene lengths here - this requires ensembl ID and transcript ID
				print $out join("\t",$key,"NA","NA",$F[$i],"pathogenic","0","0",$g_length{$gnames{$key}},$g_length_tot{$gnames{$key}})."\n"; # need gene lengths here - this requires ensembl ID and transcript ID
			}
		}
	}
	close($out);
