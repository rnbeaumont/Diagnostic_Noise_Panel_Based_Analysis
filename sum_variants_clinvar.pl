...
for(my $i=1;$i<23;$i++){	# loop over the autosomes
	...
	while(<$in>){
		...
		if(!exists $conflicting{$F[0]}){
			# initialise hashes
			$conflicting{$F[0]}=0;
			$benign{$F[0]}=0;
			$pathogenic{$F[0]}=0;
			$path_and_benign{$F[0]}=0;
			$uncertain{$F[0]}=0;
			$count_variants{$F[0]}=0;
			$count{$F[0]}=0;
			$count_genes{$F[0]}=0;
		}
		for(my $i=2;$i<@H;$i+=5){	# now loop over all the genes, and we can pull in the benign and conflicting counts together
			my @G=split("_",$H[$i]);
			my $name=$G[0]."_".$G[1]."_".$G[2];
			if($G[2] eq "monoallelic"){
				if($ghash{$name."_conflicting"}>0){
					$conflicting{$F[0]}+=$ghash{$name."_conflicting"};
				}elsif($ghash{$name."_benign"}>0){
					$benign{$F[0]}+=$ghash{$name."_benign"};
				}elsif($ghash{$name."_pathogenic"}>0){
					$pathogenic{$F[0]}+=$ghash{$name."_pathogenic"};
				}elsif($ghash{$name."_path-and-benign"}>0){
					$path_and_benign{$F[0]}+=$ghash{$name."_path-and-benign"};
				}elsif($ghash{$name."_uncertain"}>0){
					$pathogenic{$F[0]}+=$ghash{$name."_uncertain"};
				}
				$count{$F[0]}+=$ghash{$name."_conflicting"}+$ghash{$name."_benign"}+$ghash{$name."_pathogenic"}+$ghash{$name."_path-and-benign"}+$ghash{$name."_uncertain"};
				if(($ghash{$name."_conflicting"}+$ghash{$name."_benign"}+$ghash{$name."_pathogenic"}+$ghash{$name."_path-and-benign"})+$ghash{$name."_uncertain"}>0){
					$count_genes{$F[0]}++;
				}
			}elsif($G[2] eq "biallelic"){
				if($ghash{$name."_conflicting"}>1){
					$conflicting{$F[0]}+=$ghash{$name."_conflicting"};
				}elsif($ghash{$name."_benign"}>1){
					$benign{$F[0]}+=$ghash{$name."_benign"};
				}elsif($ghash{$name."_pathogenic"}>1){
					$pathogenic{$F[0]}+=$ghash{$name."_pathogenic"};
				}elsif($ghash{$name."_path-and-benign"}>1){
					$path_and_benign{$F[0]}+=$ghash{$name."_path-and-benign"};
				}elsif($ghash{$name."_uncertain"}>1){
					$pathogenic{$F[0]}+=$ghash{$name."_uncertain"};
				}
				$count{$F[0]}+=$ghash{$name."_conflicting"}+$ghash{$name."_benign"}+$ghash{$name."_pathogenic"}+$ghash{$name."_path-and-benign"}+$ghash{$name."_uncertain"};
				if(($ghash{$name."_conflicting"}+$ghash{$name."_benign"}+$ghash{$name."_pathogenic"}+$ghash{$name."_path-and-benign"}+$ghash{$name."_uncertain"})>1){
					$count_genes{$F[0]}++;
				}
			}else{
				$count{$F[0]}+=0;
...
print $out "ID\tcount_variants_unique\tcount_variants\tcount_genes\tcount_homs\tconflicting\tbenign\tpathogenic\tpath_and_benign\tuncertain\n";
foreach my $key (keys %count){
	...
	print $out join("\t","\t".$conflicting{$key},$benign{$key},$pathogenic{$key},$path_and_benign{$key},$uncertain{$key}."\n");
}
close($out);
