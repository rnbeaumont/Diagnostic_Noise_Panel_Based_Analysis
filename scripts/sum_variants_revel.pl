	...
	while(<$in>){
		...
		for(my $i=2;$i<@H;$i+=4){	# now loop over all the genes, and we can pull in the mis and lof counts together
			...
			if($G[2] eq "monoallelic"){
				if($ghash{$name."_5"}>0){
					$lof{$F[0]}+=$ghash{$name."_5"};
				}elsif($ghash{$name."_4"}>0){
					$mis{$F[0]}+=$ghash{$name."_4"};
				}elsif($ghash{$name."_3"}>0){
					$low{$F[0]}+=$ghash{$name."_3"};
				}elsif($ghash{$name."_2"}>0){
					$mod{$F[0]}+=$ghash{$name."_2"};
				}elsif($ghash{$name."_1"}>0){
					$unk{$F[0]}+=$ghash{$name."_1"};
				}elsif($ghash{$name."_0"}>0){
					$zero{$F[0]}+=$ghash{$name."_0"};
				}
				$count{$F[0]}+=$ghash{$name."_5"}+$ghash{$name."_4"}+$ghash{$name."_3"}+$ghash{$name."_2"};
				if(($ghash{$name."_5"}+$ghash{$name."_4"}+$ghash{$name."_3"}+$ghash{$name."_2"})>0){
					$count_genes{$F[0]}++;
				}
			}elsif($G[2] eq "biallelic"){
				if($ghash{$name."_5"}>1){
					$lof{$F[0]}+=$ghash{$name."_5"};
				}elsif($ghash{$name."_4"}>1){
					$mis{$F[0]}+=$ghash{$name."_4"}+$ghash{$name."_5"};
				}elsif($ghash{$name."_3"}>1){
					$low{$F[0]}+=$ghash{$name."_3"}+$ghash{$name."_4"}+$ghash{$name."_5"};
				}elsif($ghash{$name."_2"}>1){
					$mod{$F[0]}+=$ghash{$name."_2"}+$ghash{$name."_3"}+$ghash{$name."_4"}+$ghash{$name."_5"};
				}elsif($ghash{$name."_1"}>1){
					$unk{$F[0]}+=$ghash{$name."_1"}+$ghash{$name."_2"}+$ghash{$name."_3"}+$ghash{$name."_4"}+$ghash{$name."_5"};
				}elsif($ghash{$name."_0"}>1){
					$zero{$F[0]}+=$ghash{$name."_0"}+$ghash{$name."_1"}+$ghash{$name."_2"}+$ghash{$name."_3"}+$ghash{$name."_4"}+$ghash{$name."_5"};
				}
				$count{$F[0]}+=$ghash{$name."_5"}+$ghash{$name."_4"}+$ghash{$name."_3"}+$ghash{$name."_2"}+$ghash{$name."_1"}+$ghash{$name."_0"};
				if(($ghash{$name."_5"}+$ghash{$name."_4"}+$ghash{$name."_3"}+$ghash{$name."_2"}+$ghash{$name."_1"}+$ghash{$name."_0"})>1){
					$count_genes{$F[0]}++;
				}
			}else{
				$count{$F[0]}+=0;
...
print $out "ID\tcount_variants_unique\tcount_variants\tcount_genes\tcount_homs\trevel_5\trevel_4\trevel_3\trevel_2\trevel_1\trevel_0\n";
foreach my $key (keys %count){
	...
	print $out join("\t","\t".$lof{$key},$mis{$key},$low{$key},$mod{$key},$unk{$key},$zero{$key}."\n");
}
close($out);
