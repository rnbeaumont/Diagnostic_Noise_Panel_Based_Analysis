...
for(my $i=1;$i<23;$i++){	# loop over the autosomes
	open($in,"results/chr_${i}_variants_${gene_list}_all_transcripts_cache.counts") or die $!;
	<$in>;	# skip the header
	while(<$in>){
		chomp;
		my @F=split(' ');
	...
	close($in);
	open($in,"results/chr_${i}_variants_${gene_list}_all_transcripts_cache.variant_info") or die $!;
	<$in>;
	while(<$in>){
		chomp;
		my @F=split(' ',$_,2);
...
print $out "ID\tcount_variants_unique\tcount_variants\tcount_genes\tcount_homs\n";
