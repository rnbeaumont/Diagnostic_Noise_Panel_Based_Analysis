	...
	#        if($F[3] eq "is_canonical"){	# check whether it's labelled as in the canonical transcript
	    ...
	    for(my $i=0;$i<@H;$i++){
	            ...
	            if(!exists $snp_hash{$G[0].":".$pos.":I:".$nins}){
                        $snp_hash{$G[0].":".$pos.":I:".$nins}=$F[1];
			$nins--;
                        $snp_hash{$G[0].":".$G[1].":I:".$nins}=$F[1];
                    }else{
                        $snp_hash{$G[0].":".$pos.":I:".$nins}.="\t".$F[1];
			$nins--;
                        $snp_hash{$G[0].":".$G[1].":I:".$nins}.="\t".$F[1];
                    ...
                    if(!exists $snp_hash{$G[0].":".$G[1].":".$G[3].":".$G[4]}){
                        $snp_hash{$G[0].":".$G[1].":".$G[3].":".$G[4]}=$F[1];	# so save whether it's het or hom to the hash
                    }else{
                        $snp_hash{$G[0].":".$G[1].":".$G[3].":".$G[4]}.="\t".$F[1];
                    }
                }
            }
	    #}
