...
sub MainSub{
	...
	my $individuals	= $_[2];
        my $glist       = $_[3];
	my $g_pos       = "/slade/home/rnb203/Projects/Rare_Variant_Projects/VEP_G2P/ensembl_gene_transcript_lengths";
	my $g_pos_total = "/slade/home/rnb203/Projects/Rare_Variant_Projects/VEP_G2P/ensembl_gene_positions";
	...
	open(my $out,">",$outfile);
	print $out "gene_name\tgene_id\ttranscript_id\tinheritance\trevel_quintile\tvariants\tindividuals\tlength\ttotal_length\n";
	# and loop through the chromosomes, reading the variant info for each
	my %genes_found=();
	for(my $i=1;$i<23;$i++){
		ReadFile($infile,$out,$i,\%g_length,\%g_length_tot,\%to_include,\%genes_found,\%gnames);
	}
	...
	foreach my $key (keys %panel_genes){
		...
		for(my $i=0;$i<@F;$i++){
			if(!exists $genes_found{$key."_".$F[$i]} && ($F[$i] eq "monoallelic" || $F[$i] eq "biallelic")){
				print $out join("\t",$key,"NA","NA",$F[$i],"0","0","0",$g_length{$gnames{$key}},$g_length_tot{$gnames{$key}})."\n"; # need gene lengths here - this requires ensembl ID and transcript ID
			}
		}
	}
	close($out);
