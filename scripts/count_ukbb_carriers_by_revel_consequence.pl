#!/usr/bin/perl -w
# script to read in VEP reports, and list the number of G2P variants are carried by each UKBB individual, separated by REVEL consequence quantiles
use strict;
use Getopt::Long;
use POSIX;
...
if ($vep && $plink && $out && $af_file_ukb && $scores && $cutoffs) {
    &MainSub($vep, $plink, $out, $af_file_ukb,$scores,$cutoffs);
}
else {
    ...
    print "        --cutoffs   file containing cutoffs for REVEL score quintiles\n\n";
}
...
sub MainSub{
    ...
    # Read in the REVEL scores split by quintiles
    my ($revel_1,$revel_2,$revel_3,$revel_4,$revel_5)=&ReadREVEL($scores,$cutoffs);
    ...
    &ReadPlink($plink,$snp_hash,$out,$genes,$genes_hash,$freq_ukb,$allele_ukb,$other_allele_ukb,$revel_1,$revel_2,$revel_3,$revel_4,$revel_5);
}
...
sub ReadPlink{
    ...
    my $quint1        = $_[8]; # hash containing SNPs in REVEL quintile 1
    my $quint2        = $_[9]; # hash containing SNPs in REVEL quintile 2
    my $quint3        = $_[10]; # hash containing SNPs in REVEL quintile 3
    my $quint4        = $_[11]; # hash containing SNPs in REVEL quintile 4
    my $quint5        = $_[12]; # hash containing SNPs in REVEL quintile 5
	...
	my $frq=0;
	    ...
	    # find out which quintile this SNP is in
	    my @H=split(":",$F[1]);
	    my $quint=0;
	    if(exists $$quint1{$H[0].":".$H[1]}){
		    $quint=1;
	    }elsif(exists $$quint2{$H[0].":".$H[1]}){
		    $quint=2;
	    }elsif(exists $$quint3{$H[0].":".$H[1]}){
		    $quint=3;
	    }elsif(exists $$quint4{$H[0].":".$H[1]}){
		    $quint=4;
	    }elsif(exists $$quint5{$H[0].":".$H[1]}){
		    $quint=5;
	    }
		...
		for(my $i=0;$i<@H;$i++){
                    if(($H[$i] eq "biallelic" && $frq<$biallelic_freq) || ($H[$i] eq "monoallelic" && $frq<$mono_freq)){	# check the allele frequency is OK, and if so count the carriers
                        for(my $j=0;$j<@iid;$j++){	# loop over the individuals and append the variants info for carriers to  an array, as well as counting the number if variants they have
                            $gene_variant{$key."_".$H[$i]."_".$iid[$j]."_".$quint}+=$G[$j];	# increment the number of variants in this gene for this person. Name is gene_interitance-mode_ID
	                    if($G[$j]>0){
	...
	for(my $j=0;$j<@F;$j++){
            if($F[$j] eq "monoallelic"){
	        for(my $k=0;$k<=5;$k++){
	            print $out_file "\t".$$genes[$i]."_".$F[$j]."_".$k;
                }
	    }elsif($F[$j] eq "biallelic"){
	        for(my $k=5;$k>=0;$k--){
	            print $out_file "\t".$$genes[$i]."_".$F[$j]."_".$k;
                }
	    }
	...
	# now loop through each gene unit and output it
		...
		for(my $k=0;$k<@F;$k++){
			if($F[$k] eq "monoallelic"){
				for(my $l=0;$l<=5;$l++){
	            			if(exists $gene_variant{$$genes[$j]."_monoallelic_".$iid[$i]."_".$l}){
				        	print $out_file "\t".$gene_variant{$$genes[$j]."_monoallelic_".$iid[$i]."_".$l};
					}else{
						print $out_file "\t0";
					}
				}
			...
			if($F[$k] eq "biallelic"){
				...
				my $max_cons=0;
				for(my $l=5;$l>=0;$l--){
            				if(exists $gene_variant{$$genes[$j]."_biallelic_".$iid[$i]."_".$l}){
						$total_vars+=$gene_variant{$$genes[$j]."_biallelic_".$iid[$i]."_".$l};
						if($total_vars>1 && $max_cons==0){
		    					print $out_file "\t".$total_vars;
							$max_cons=1;
						}else{
							print $out_file "\t0";
						}
...
sub ReadREVEL{	# Subroutine to read in REVEL quintile boundaries, then read in a list of SNPs with REVEL scores and split them into hashes based on quintile
	...
	my %quint5     = ();
	my @bounds     = ();
	open(my $in,$cutoffs) or die $!;	# read in the cutoffs
	while(<$in>){
		chomp;
		my @F=split(' ');
		push @bounds,$F[0];
	...
	close($in);
	# now read the REVEL scores
	open($in,$scores) or die $!;
	while(<$in>){
		chomp;
		my @F=split(' ');
		if($F[2]<=$bounds[0]){
	...
	close($in);
	return(\%quint1,\%quint2,\%quint3,\%quint4,\%quint5);
}
