...
if ($vep && $plink && $out && $af_file_ukb) {
    &MainSub($vep, $plink, $out, $af_file_ukb);
}
else {
...
sub MainSub{
    ...
    my %modifier=();
    my %low=();
    $lof{"frameshift_variant"}=1;
    $lof{"splice_acceptor_variant"}=1;
    $lof{"splice_donor_variant"}=1;
    $lof{"start_lost"}=1;
    $lof{"stop_gained"}=1;
    $lof{"stop_lost"}=1;
    $missense{"missense_variant"}=1;
    $missense{"inframe_deletion"}=1;
    $missense{"inframe_insertion"}=1;
    $missense{"protein_altering_variant"}=1;
    $missense{"protein_altering_variant"}=1;
    $missense{"coding_sequence_variant"}=1;
    $low{"incomplete_terminal_codon_variant"}=1;
    $low{"splice_region_variant"}=1;
    $modifier{"3_prime_UTR_variant"}=1;
    $modifier{"5_prime_UTR_variant"}=1;
    $modifier{"intron_variant"}=1;
    $modifier{"NMD_transcript_variant"}=1;
    $modifier{"non_coding_transcript_exon_variant"}=1;
    $modifier{"non_coding_transcript_variant"}=1;
    ...
    print "Reading gnomad allele frequencies\n";
    ...
    &ReadPlink($plink,$snp_hash,$out,$genes,$genes_hash,$freq_ukb,$allele_ukb,$other_allele_ukb,\%missense,\%lof,$snp_annotation,\%low,\%modifier);
}
...
sub ReadPlink{
    ...
    my $missense      = $_[8];	# hash of annotations classed as missense
    my $lof           = $_[9];	# hash of annotations classed as lof
    my $snp_annotation = $_[10];	# hash containing the annotations for a SNP
    my $low           = $_[11];	# hash of annotations classed as low
    my $modifier      = $_[12];	# hash of annotations classed as modifier
	...
	my $frq=0;
	    ...
	    my @Z=split(",",$snp_annotation->{$F[1]});	# split out the annotations for this SNP and decide if it's LOF or missense
	    my $lof_seen=0;	# flag to say if this SNP is lof
	    my $mis_seen=0;	# flag to say if this SNP is missense
	    my $low_seen=0;	# flag to say if this SNP is low
	    my $mod_seen=0;	# flag to say if this SNP is modifier
	    for(my $i=0;$i<@Z;$i++){
		    if(exists $missense->{$Z[$i]}){
			    $mis_seen=1;
		    }
		    if(exists $lof->{$Z[$i]}){
			    $lof_seen=1;
	...
	for(my $j=0;$j<@F;$j++){
            print $out_file "\t".$$genes[$i]."_".$F[$j]."_lof";
            print $out_file "\t".$$genes[$i]."_".$F[$j]."_missense";
            print $out_file "\t".$$genes[$i]."_".$F[$j]."_low";
            print $out_file "\t".$$genes[$i]."_".$F[$j]."_modifier";
	}
    }
    print $out_file "\n";
	...
	if(exists  $person_variant{$iid[$i]}){	# if the person has any variants add the ones in valid units to a hash so we only count each variant once
		...
		if(exists $person_variant{$$genes[$j]."_monoallelic_".$iid[$i]."_lof"}){	# monoallelic count all variants
		    my @F=split(",",$person_variant{$$genes[$j]."_monoallelic_".$iid[$i]."_lof"});
		    for(my $k=0;$k<@F;$k++){
			my @G=split("_",$F[$k]);
		        $variants_valid{$G[0]}=$G[1];
		    }
		}
		if(exists $person_variant{$$genes[$j]."_monoallelic_".$iid[$i]."_missense"}){	# monoallelic count all variants
		    my @F=split(",",$person_variant{$$genes[$j]."_monoallelic_".$iid[$i]."_missense"});
		    for(my $k=0;$k<@F;$k++){
			my @G=split("_",$F[$k]);
		        $variants_valid{$G[0]}=$G[1];
		    }
		}
		if(exists $person_variant{$$genes[$j]."_monoallelic_".$iid[$i]."_low"}){	# monoallelic count all variants
		    my @F=split(",",$person_variant{$$genes[$j]."_monoallelic_".$iid[$i]."_low"});
		    for(my $k=0;$k<@F;$k++){
			my @G=split("_",$F[$k]);
		        $variants_valid{$G[0]}=$G[1];
		    }
		}
		if(exists $person_variant{$$genes[$j]."_monoallelic_".$iid[$i]."_modifier"}){	# monoallelic count all variants
		    my @F=split(",",$person_variant{$$genes[$j]."_monoallelic_".$iid[$i]."_modifier"});
		    for(my $k=0;$k<@F;$k++){
			my @G=split("_",$F[$k]);
		        $variants_valid{$G[0]}=$G[1];
		    }
		}
		if(exists $person_variant{$$genes[$j]."_biallelic_".$iid[$i]."_lof"}){	# biallelic so only count variants if there is more than 1
		    if($gene_variant{$$genes[$j]."_biallelic_".$iid[$i]}>1){
		        my @F=split(",",$person_variant{$$genes[$j]."_biallelic_".$iid[$i]."_lof"});
		        for(my $k=0;$k<@F;$k++){
			    my @G=split("_",$F[$k]);
		            $variants_valid{$G[0]}=$G[1];
		        }
		...
		if(exists $person_variant{$$genes[$j]."_biallelic_".$iid[$i]."_missense"}){	# biallelic so only count variants if there is more than 1
		    if($gene_variant{$$genes[$j]."_biallelic_".$iid[$i]}>1){
		        my @F=split(",",$person_variant{$$genes[$j]."_biallelic_".$iid[$i]."_missense"});
		        for(my $k=0;$k<@F;$k++){
			    my @G=split("_",$F[$k]);
		            $variants_valid{$G[0]}=$G[1];
		        }
		...
		if(exists $person_variant{$$genes[$j]."_biallelic_".$iid[$i]."_low"}){	# biallelic so only count variants if there is more than 1
		    if($gene_variant{$$genes[$j]."_biallelic_".$iid[$i]}>1){
		        my @F=split(",",$person_variant{$$genes[$j]."_biallelic_".$iid[$i]."_low"});
		        for(my $k=0;$k<@F;$k++){
			    my @G=split("_",$F[$k]);
		            $variants_valid{$G[0]}=$G[1];
		        }
		...
		if(exists $person_variant{$$genes[$j]."_biallelic_".$iid[$i]."_modifier"}){	# biallelic so only count variants if there is more than 1
		    if($gene_variant{$$genes[$j]."_biallelic_".$iid[$i]}>1){
		        my @F=split(",",$person_variant{$$genes[$j]."_biallelic_".$iid[$i]."_modifier"});
		        for(my $k=0;$k<@F;$k++){
			    my @G=split("_",$F[$k]);
		            $variants_valid{$G[0]}=$G[1];
		        }
	...
	# now loop through each gene unit and output it
		...
		for(my $k=0;$k<@F;$k++){
			if($F[$k] eq "monoallelic"){
            			if(exists $gene_variant{$$genes[$j]."_monoallelic_".$iid[$i]."_lof"}){
			        	print $out_file "\t".$gene_variant{$$genes[$j]."_monoallelic_".$iid[$i]."_lof"};
				}else{
					print $out_file "\t0";
				}
            			if(exists $gene_variant{$$genes[$j]."_monoallelic_".$iid[$i]."_missense"}){
			        	print $out_file "\t".$gene_variant{$$genes[$j]."_monoallelic_".$iid[$i]."_missense"};
				}else{
					print $out_file "\t0";
				}
            			if(exists $gene_variant{$$genes[$j]."_monoallelic_".$iid[$i]."_low"}){
			        	print $out_file "\t".$gene_variant{$$genes[$j]."_monoallelic_".$iid[$i]."_low"};
				}else{
					print $out_file "\t0";
				}
            			if(exists $gene_variant{$$genes[$j]."_monoallelic_".$iid[$i]."_modifier"}){
			        	print $out_file "\t".$gene_variant{$$genes[$j]."_monoallelic_".$iid[$i]."_modifier"};
				}else{
					print $out_file "\t0";
				}
			}
			if($F[$k] eq "biallelic"){
            			if(exists $gene_variant{$$genes[$j]."_biallelic_".$iid[$i]."_lof"}){
					if($gene_variant{$$genes[$j]."_biallelic_".$iid[$i]}>1){
		    				print $out_file "\t".$gene_variant{$$genes[$j]."_biallelic_".$iid[$i]."_lof"};
					}else{
						print $out_file "\t0";
					}
				}else{
            			...
            			if(exists $gene_variant{$$genes[$j]."_biallelic_".$iid[$i]."_missense"}){
					if($gene_variant{$$genes[$j]."_biallelic_".$iid[$i]}>1){
		    				print $out_file "\t".$gene_variant{$$genes[$j]."_biallelic_".$iid[$i]."_missense"};
					}else{
						print $out_file "\t0";
					}
				}else{
            			...
            			if(exists $gene_variant{$$genes[$j]."_biallelic_".$iid[$i]."_low"}){
					if($gene_variant{$$genes[$j]."_biallelic_".$iid[$i]}>1){
		    				print $out_file "\t".$gene_variant{$$genes[$j]."_biallelic_".$iid[$i]."_low"};
					}else{
						print $out_file "\t0";
					}
				}else{
            			...
            			if(exists $gene_variant{$$genes[$j]."_biallelic_".$iid[$i]."_modifier"}){
					if($gene_variant{$$genes[$j]."_biallelic_".$iid[$i]}>1){
		    				print $out_file "\t".$gene_variant{$$genes[$j]."_biallelic_".$iid[$i]."_modifier"};
					}else{
						print $out_file "\t0";
					}
				}else{
			...
			if($F[$k] eq "imprinted"){
            			if(exists $gene_variant{$$genes[$j]."_imprinted_".$iid[$i]."_lof"}){
					if($gene_variant{$$genes[$j]."_imprinted_".$iid[$i]}>1){
		    				print $out_file "\t".$gene_variant{$$genes[$j]."_imprinted_".$iid[$i]."_lof"};
					}else{
						print $out_file "\t0";
					}
				}else{
            			...
            			if(exists $gene_variant{$$genes[$j]."_imprinted_".$iid[$i]."_missense"}){
					if($gene_variant{$$genes[$j]."_imprinted_".$iid[$i]}>1){
		    				print $out_file "\t".$gene_variant{$$genes[$j]."_imprinted_".$iid[$i]."_missense"};
					}else{
						print $out_file "\t0";
					}
				}else{
            			...
            			if(exists $gene_variant{$$genes[$j]."_imprinted_".$iid[$i]."_low"}){
					if($gene_variant{$$genes[$j]."_imprinted_".$iid[$i]}>1){
		    				print $out_file "\t".$gene_variant{$$genes[$j]."_imprinted_".$iid[$i]."_low"};
					}else{
						print $out_file "\t0";
					}
				}else{
            			...
            			if(exists $gene_variant{$$genes[$j]."_imprinted_".$iid[$i]."_modifier"}){
					if($gene_variant{$$genes[$j]."_imprinted_".$iid[$i]}>1){
		    				print $out_file "\t".$gene_variant{$$genes[$j]."_imprinted_".$iid[$i]."_modifier"};
					}else{
						print $out_file "\t0";
					}
				}else{
			...
			if($F[$k] eq "mosaic"){
            			if(exists $gene_variant{$$genes[$j]."_mosaic_".$iid[$i]."_lof"}){
					if($gene_variant{$$genes[$j]."_mosaic_".$iid[$i]}>1){
		    				print $out_file "\t".$gene_variant{$$genes[$j]."_mosaic_".$iid[$i]."_lof"};
					}else{
						print $out_file "\t0";
					}
				}else{
            			...
            			if(exists $gene_variant{$$genes[$j]."_mosaic_".$iid[$i]."_missense"}){
					if($gene_variant{$$genes[$j]."_mosaic_".$iid[$i]}>1){
		    				print $out_file "\t".$gene_variant{$$genes[$j]."_mosaic_".$iid[$i]."_missense"};
					}else{
						print $out_file "\t0";
					}
				}else{
            			...
            			if(exists $gene_variant{$$genes[$j]."_mosaic_".$iid[$i]."_low"}){
					if($gene_variant{$$genes[$j]."_mosaic_".$iid[$i]}>1){
		    				print $out_file "\t".$gene_variant{$$genes[$j]."_mosaic_".$iid[$i]."_low"};
					}else{
						print $out_file "\t0";
					}
				}else{
            			...
            			if(exists $gene_variant{$$genes[$j]."_mosaic_".$iid[$i]."_modifier"}){
					if($gene_variant{$$genes[$j]."_mosaic_".$iid[$i]}>1){
		    				print $out_file "\t".$gene_variant{$$genes[$j]."_mosaic_".$iid[$i]."_modifier"};
					}else{
						print $out_file "\t0";
					}
				}else{
			...
			if($F[$k] eq "digenic"){
            			if(exists $gene_variant{$$genes[$j]."_digenic_".$iid[$i]."_lof"} && $F[$k]){
					if($gene_variant{$$genes[$j]."_digenic_".$iid[$i]}>1){
		    				print $out_file "\t".$gene_variant{$$genes[$j]."_digenic_".$iid[$i]."_lof"};
					}else{
		    				print $out_file "\t0";
					}
				}else{
            			...
            			if(exists $gene_variant{$$genes[$j]."_digenic_".$iid[$i]."_missense"} && $F[$k]){
					if($gene_variant{$$genes[$j]."_digenic_".$iid[$i]}>1){
		    				print $out_file "\t".$gene_variant{$$genes[$j]."_digenic_".$iid[$i]."_missense"};
					}else{
		    				print $out_file "\t0";
					}
				}else{
            			...
            			if(exists $gene_variant{$$genes[$j]."_digenic_".$iid[$i]."_low"} && $F[$k]){
					if($gene_variant{$$genes[$j]."_digenic_".$iid[$i]}>1){
		    				print $out_file "\t".$gene_variant{$$genes[$j]."_digenic_".$iid[$i]."_low"};
					}else{
		    				print $out_file "\t0";
					}
				}else{
            			...
            			if(exists $gene_variant{$$genes[$j]."_digenic_".$iid[$i]."_modifier"} && $F[$k]){
					if($gene_variant{$$genes[$j]."_digenic_".$iid[$i]}>1){
		    				print $out_file "\t".$gene_variant{$$genes[$j]."_digenic_".$iid[$i]."_modifier"};
					}else{
		    				print $out_file "\t0";
					}
				}else{
			...
			if($F[$k] eq "uncertain"){
            			if(exists $gene_variant{$$genes[$j]."_uncertain_".$iid[$i]."_lof"}){
					if($gene_variant{$$genes[$j]."_uncertain_".$iid[$i]}>1){
		    				print $out_file "\t".$gene_variant{$$genes[$j]."_uncertain_".$iid[$i]."_lof"};
					}else{
						print $out_file "\t0";
					}
				}else{
            			...
            			if(exists $gene_variant{$$genes[$j]."_uncertain_".$iid[$i]."_missense"}){
					if($gene_variant{$$genes[$j]."_uncertain_".$iid[$i]}>1){
		    				print $out_file "\t".$gene_variant{$$genes[$j]."_uncertain_".$iid[$i]."_missense"};
					}else{
						print $out_file "\t0";
					}
				}else{
            			...
            			if(exists $gene_variant{$$genes[$j]."_uncertain_".$iid[$i]."_low"}){
					if($gene_variant{$$genes[$j]."_uncertain_".$iid[$i]}>1){
		    				print $out_file "\t".$gene_variant{$$genes[$j]."_uncertain_".$iid[$i]."_low"};
					}else{
						print $out_file "\t0";
					}
				}else{
            			...
            			if(exists $gene_variant{$$genes[$j]."_uncertain_".$iid[$i]."_modifier"}){
					if($gene_variant{$$genes[$j]."_uncertain_".$iid[$i]}>1){
		    				print $out_file "\t".$gene_variant{$$genes[$j]."_uncertain_".$iid[$i]."_modifier"};
					}else{
						print $out_file "\t0";
					}
				}else{
