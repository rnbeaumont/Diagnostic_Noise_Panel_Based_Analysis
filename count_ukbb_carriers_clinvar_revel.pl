...
my $scores = undef;	# file containing variant REVEL scores
my $cutoffs = undef;	# file containing cutoffs for REVEL quintiles
GetOptions(
  "vep=s"     => \$vep,
  "plink=s"   => \$plink,
  "clinvar=s" => \$clinvar,
...
if ($vep && $plink && $out && $af_file && $af_file_ukb && $clinvar && $scores) {
    &MainSub($vep, $plink, $out, $af_file, $af_file_ukb, $clinvar, $scores);
}
else {
    ...
    print "        --scores    file containing list of variants and their REVEL scores\n";
...
sub MainSub{
    ...
    # Read in the REVEL scores for all SNPs
    my ($revel_scores)=&ReadREVEL($scores);
    ...
    &ReadPlink($plink,$snp_hash,$out,$genes,$genes_hash,$freq_ukb,$allele_ukb,$other_allele_ukb,$clinvar_annotation,$revel_scores);
}
...
sub ReadPlink{
    ...
    my $revel_scores   = $_[9]; # hash containing SNPs and their REVEL scores
	...
	my $frq=0;
	    ...
	    # get the variant's REVEL score
	    my $rscore=0;
	    my @H=split(":",$F[1]);
	    if(exists $revel_scores->{$H[0].":".$H[1]}){
		    if($revel_scores->{$H[0].":".$H[1]}>=0.7){
			    $rscore="PASS";
		    }
	    }
		...
		for(my $i=0;$i<@H;$i++){
                    if(($H[$i] eq "biallelic" && $frq<$biallelic_freq) || ($H[$i] eq "monoallelic" && $frq<$mono_freq)){	# check the allele frequency is OK, and if so count the carriers
                        for(my $j=0;$j<@iid;$j++){	# loop over the individuals and append the variants info for carriers to  an array, as well as counting the number if variants they have
                            $gene_variant{$key."_".$H[$i]."_".$iid[$j]."_".$annot."_".$rscore}+=$G[$j];	# increment the number of variants in this gene for this person. Name is gene_interitance-mode_ID
	                    ...
	                    if($G[$j]>0){
                                ...
                                $person_variant{$key."_".$H[$i]."_".$iid[$j]."_".$annot."_".$rscore}.=$F[1]."_".$G[$j].",";	# save a list of variants for this person for each gene
			    }
			}
                    }
		}
	...
	if(exists  $person_variant{$iid[$i]}){	# if the person has any variants add the ones in valid units to a hash so we only count each variant once
		...
		for(my $c=0;$c<@C;$c++){
		    if(exists $person_variant{$$genes[$j]."_monoallelic_".$iid[$i]."_".$C[$c]."_PASS"}){	# monoallelic count all variants
		        my @F=split(",",$person_variant{$$genes[$j]."_monoallelic_".$iid[$i]."_".$C[$c]."_PASS"});
		        for(my $k=0;$k<@F;$k++){
			    my @G=split("_",$F[$k]);
		...
		for(my $c=0;$c<@C;$c++){
		    if(exists $person_variant{$$genes[$j]."_biallelic_".$iid[$i]."_".$C[$c]."_PASS"}){	# biallelic so only count variants if there is more than 1
		        if($gene_variant{$$genes[$j]."_biallelic_".$iid[$i]}>1){
		            my @F=split(",",$person_variant{$$genes[$j]."_biallelic_".$iid[$i]."_".$C[$c]."_PASS"});
		            for(my $k=0;$k<@F;$k++){
			        my @G=split("_",$F[$k]);
		                $variants_valid{$G[0]}=$G[1];
			    }
	...
	# now loop through each gene unit and output it
		...
		for(my $k=0;$k<@F;$k++){
			if($F[$k] eq "monoallelic"){
			    for(my $c=0;$c<@C;$c++){
            			if(exists $gene_variant{$$genes[$j]."_monoallelic_".$iid[$i]."_".$C[$c]."_PASS"}){
			        	print $out_file "\t".$gene_variant{$$genes[$j]."_monoallelic_".$iid[$i]."_".$C[$c]."_PASS"};
				}else{
					print $out_file "\t0";
				}
			    }
			...
			if($F[$k] eq "biallelic"){
			    for(my $c=0;$c<@C;$c++){
            			if(exists $gene_variant{$$genes[$j]."_biallelic_".$iid[$i]."_".$C[$c]."_PASS"}){
					if($gene_variant{$$genes[$j]."_biallelic_".$iid[$i]}>1){
		    				print $out_file "\t".$gene_variant{$$genes[$j]."_biallelic_".$iid[$i]."_".$C[$c]."_PASS"};
					}else{
						print $out_file "\t0";
					}
				}else{
			...
			if($F[$k] eq "imprinted"){
			    for(my $c=0;$c<@C;$c++){
            			if(exists $gene_variant{$$genes[$j]."_imprinted_".$iid[$i]."_".$C[$c]."_PASS"}){
					if($gene_variant{$$genes[$j]."_imprinted_".$iid[$i]}>1){
		    				print $out_file "\t".$gene_variant{$$genes[$j]."_imprinted_".$iid[$i]."_".$C[$c]."_PASS"};
					}else{
						print $out_file "\t0";
					}
				}else{
			...
			if($F[$k] eq "mosaic"){
			    for(my $c=0;$c<@C;$c++){
            			if(exists $gene_variant{$$genes[$j]."_mosaic_".$iid[$i]."_".$C[$c]."_PASS"}){
					if($gene_variant{$$genes[$j]."_mosaic_".$iid[$i]}>1){
		    				print $out_file "\t".$gene_variant{$$genes[$j]."_mosaic_".$iid[$i]."_".$C[$c]."_PASS"};
					}else{
						print $out_file "\t0";
					}
				}else{
			...
			if($F[$k] eq "digenic"){
			    for(my $c=0;$c<@C;$c++){
            			if(exists $gene_variant{$$genes[$j]."_digenic_".$iid[$i]."_".$C[$c]."_PASS"} && $F[$k]){
					if($gene_variant{$$genes[$j]."_digenic_".$iid[$i]}>1){
		    				print $out_file "\t".$gene_variant{$$genes[$j]."_digenic_".$iid[$i]."_".$C[$c]."_PASS"};
					}else{
		    				print $out_file "\t0";
					}
				}else{
			...
			if($F[$k] eq "uncertain"){
			    for(my $c=0;$c<@C;$c++){
            			if(exists $gene_variant{$$genes[$j]."_uncertain_".$iid[$i]."_".$C[$c]."_PASS"}){
					if($gene_variant{$$genes[$j]."_uncertain_".$iid[$i]}>1){
		    				print $out_file "\t".$gene_variant{$$genes[$j]."_uncertain_".$iid[$i]."_".$C[$c]."_PASS"};
					}else{
						print $out_file "\t0";
					}
				}else{
...
sub ReadREVEL{  # Subroutine to read in REVEL quintile boundaries, then read in a list of SNPs with REVEL scores and split them into hashes based on quintile
        my $scores     = $_[0];
        my %scores     = ();
        # read and save the REVEL scores
        open(my $in,$scores) or die $!;
        while(<$in>){
                chomp;
                my @F=split(' ');
		$scores{$F[0]}=$F[2];
